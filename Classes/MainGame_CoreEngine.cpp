...
void MainGame::SetCirclePosition(NodeInfo* nodeinfo)
{
	...
	if(nodeinfo -> result == Score::NONE)
	{
		...
		if(!(rh.type & RHYTHMTYPE_LONG))
		{
			...
			float Delta = curTime + speed - rh.beginTime;	//note从出生到miss一共要经历 speed + misstime, Delta为note自出生到当前经过的时间

			float scale = Delta / speed;	//根据scale设置当前的note的大小以及位置
			sp->setScale(scale);
			...
			if(nodeinfo -> result != Score::NONE && nodeinfo -> result != Score::MISS)	//如果得到了评价
			{
				showScoreEffect(nodeinfo -> result);//显示打击效果并将node设为不可见，从scene当中移去
